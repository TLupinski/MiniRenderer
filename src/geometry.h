...
template <class t> struct Vec4 {
    ...
    Vec4<t> operator ^(const Vec3<t> &v) const { return Vec4<t>(y*v.z-z*v.y, z*v.x-x*v.z, x*v.y-y*v.x); }
    Vec4<t> operator +(const Vec3<t> &v) const { return Vec4<t>(x+v.x, y+v.y, z+v.z, w+v.w); }
    Vec4<t> operator -(const Vec3<t> &v) const { return Vec4<t>(x-v.x, y-v.y, z-v.z, w-v.w); }
...
extern Vec4f matrixToVector(Matrix m);

extern Matrix vectorToMatrix(Vec4f v);
